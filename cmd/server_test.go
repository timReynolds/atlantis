...
var testFlags = map[string]any{
	...
	RunStoreRetentionDaysFlag:        0,
	RunStoreTypeFlag:                 "noop",
...
func TestExecute_ReplicaRoutingValidation(t *testing.T) {
	valid := func() map[string]any {
		return map[string]any{
			...
			LockingDBType:            "redis",
			RedisHost:                "redis.example.com",
			ReplicaAdvertiseURLFlag:  "http://replica-0.atlantis-headless:4141",
			InternalCommandTokenFlag: "internal-token",
			OwnershipTTLSecondsFlag:  30,
		}
	}
	...
	tests := []struct {
		...
		},
		{
			...
			expected: "--replica-advertise-url must be an absolute HTTP(S) origin without credentials, path, query, or fragment",
		},
		{
			name: "rejects advertise URL base path",
			configure: func(flags map[string]any) {
				flags[ReplicaAdvertiseURLFlag] = "http://replica-0:4141/atlantis"
			},
			expected: "--replica-advertise-url must be an absolute HTTP(S) origin without credentials, path, query, or fragment",
		},
		{
