...
func TestRunHistoryRecordsProjectsCancelledBeforeExecution(t *testing.T) {
	writer := &recordingRunWriter{}
	history := newTestRunHistory(t, writer)
	ctx := testRunContext(t)
	lifecycle := history.Begin(ctx, runs.CommandPlan, runs.TriggerComment)
	projectCtx := command.ProjectContext{
		RunID: ctx.RunID, ObserveProjectResult: ctx.ObserveProjectResult,
		CommandName: command.Plan, ProjectName: "network", RepoRelDir: "network",
		Workspace: "default", Log: ctx.Log,
	}

	results := createCancelledResults([][]command.ProjectContext{{projectCtx}})
	lifecycle.Finish()

	require.True(t, results[0].Cancelled)
	require.True(t, ctx.CommandCancelled)
	require.Len(t, writer.projectsCreated, 1)
	require.Len(t, writer.projectsCompleted, 1)
