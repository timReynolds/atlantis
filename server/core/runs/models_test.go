...
func TestRunValidate(t *testing.T) {
	...
	zero := time.Time{}
	run.StartedAt = &zero
	Assert(t, run.Validate() != nil, "terminal run with zero started time must be rejected")
...
func TestProjectRunValidatePreservesAllPlanCounts(t *testing.T) {
	...
	zero := time.Time{}
	projectRun.StartedAt = &zero
	Assert(t, projectRun.Validate() != nil, "terminal project run with zero started time must be rejected")
	projectRun.StartedAt = &started
