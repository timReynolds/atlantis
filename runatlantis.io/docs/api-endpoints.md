...
If execution completes but Atlantis cannot persist the final remediation history record, the response retains the actual execution status and includes `persistence_warning`. In particular, do not automatically retry an apply in response to this warning: infrastructure changes may already have completed.
