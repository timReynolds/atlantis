...
Absolute internal HTTP(S) origin used by other replicas to forward commands to this replica. Configuring it activates replica routing and requires Redis locking plus `--internal-command-token`. The URL must not include credentials, a path, a query, or a fragment. In Kubernetes, use a stable per-pod DNS name rather than the load-balanced Service address.
